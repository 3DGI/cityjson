...
This document records the current execution model for `cityjson-arrow` and
`cityjson-parquet`.
...
- `cityjson_arrow::ModelEncoder` and `cityjson_arrow::ModelDecoder` own the live Arrow
  IPC stream boundary
- `cityjson_parquet::PackageWriter` and `cityjson_parquet::PackageReader` own the
  persistent single-file package boundary
- `cityjson_arrow::internal` keeps doc-hidden conversion and transport helpers for
