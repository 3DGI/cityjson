# CityJSON Parquet Package Layout Specification
...
packages encoded as Parquet files.

The logical package schema is shared with the Arrow IPC layout specification.
The physical encoding changes only the table file format and file extension.
...
- The manifest `table_encoding` MUST be `parquet`.
- Every canonical table referenced by the manifest MUST be stored as a Parquet
  file.
...
Each canonical table is stored as a standalone Parquet file.

Implementations MAY split a table into multiple row groups. Readers MUST treat
the file contents as a table with the schema recorded in the Parquet footer.

The Parquet encoding changes the file format only. It does not change the
