...
mod tests {
    use super::*;
    use arrow::array::{Array, FixedSizeListArray, ListArray, DictionaryArray, AsArray, ArrayAccessor};
    use arrow::datatypes::Int8Type;
    use cityjson::prelude::{
        AttributeValue, BBox, OwnedAttributes, ResourceId32,
        ResourcePool,
    };
    use cityjson::v2_0::{CityObjects, CityObjectType, CityObject, OwnedCityObjects};
    ...
    fn test_cityobjects_to_arrow() {
        ...
        attributes.insert("name".to_string(), AttributeValue::String("Main Tower".to_string()));
        ...
        let batch = cityobjects_to_arrow(&cityobjects).expect("Failed to convert cityobjects to Arrow");

        // Verify basic structure
        assert_eq!(batch.num_rows(), 2, "Batch should have 2 rows (one per city object)");
        assert_eq!(batch.num_columns(), 9, "Batch should have 9 columns as per schema");

        // Verify IDs (first column should be the object IDs)
        let id_array = batch.column(0).as_any().downcast_ref::<arrow::array::UInt32Array>()
        ...
        let extension_array = batch.column(2).as_any().downcast_ref::<arrow::array::StringArray>()
            .expect("Third column should be StringArray of extension values");
        assert!(extension_array.is_null(0), "Building should have null extension value");
        assert_eq!(extension_array.value(1), "+CustomFeature", "Extension object should have correct value");

        // Verify geometries
        let geometries_array = batch.column(3).as_any().downcast_ref::<ListArray>()
            .expect("Fourth column should be ListArray of geometry references");
        assert!(!geometries_array.is_null(0), "Building should have geometries");

        // Get the underlying UInt32Array of geometry references for the building
        let building_geometries = geometries_array.value(0);
        let geom_ids = building_geometries.as_any().downcast_ref::<arrow::array::UInt32Array>()
            .expect("Geometry list should contain UInt32Array");
        assert_eq!(geom_ids.len(), 2, "Building should have 2 geometry references");
        assert_eq!(geom_ids.value(0), 1);
        assert_eq!(geom_ids.value(1), 2);

        // Verify geographical extent
        let geo_extent_array = batch.column(5).as_any().downcast_ref::<FixedSizeListArray>()
            .expect("Sixth column should be FixedSizeListArray for geographical extent");

        assert!(!geo_extent_array.is_null(0), "Building should have geographical extent");
        assert!(geo_extent_array.is_null(1), "Custom object should have null geographical extent");

        // Get the values of the geographical extent
        let extent_values = geo_extent_array.value(0);
        let extent_array = extent_values.as_any().downcast_ref::<arrow::array::Float64Array>()
            .expect("Geographical extent should contain Float64Array");

        assert_eq!(extent_array.len(), 6, "Geographical extent should have 6 values");
        ...
        let children_array = batch.column(6).as_any().downcast_ref::<ListArray>()
            .expect("Seventh column should be ListArray of children references");

        assert!(children_array.is_null(0), "Building should have no children");
        assert!(!children_array.is_null(1), "Custom object should have children");

        let custom_children = children_array.value(1);
        let child_ids = custom_children.as_any().downcast_ref::<arrow::array::UInt32Array>()
        ...
        let parents_array = batch.column(7).as_any().downcast_ref::<ListArray>()
            .expect("Eighth column should be ListArray of parent references");

        // The building has no parents explicitly set in our test
        assert!(parents_array.is_null(0) || parents_array.value(0).len() == 0,
