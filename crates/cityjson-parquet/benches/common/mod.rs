...
pub(crate) struct PreparedReadCase {
    ...
    pub(crate) stream_bytes_len: u64,
    pub(crate) json_bytes_len: u64,
    // Keep the NamedTempFile alive for the duration of the benchmark.
    _package_file: NamedTempFile,
}
    ...
    let metadata = SuiteMetadata {
        ...
        cases: prepared
            ...
            .map(|case| CaseMetadata {
                ...
                input_bytes: Some(case.package_bytes),
                benchmark_bytes: BTreeMap::from([
                    (READ_BENCH_PACKAGE.to_owned(), case.package_bytes),
                    (READ_BENCH_STREAM.to_owned(), case.stream_bytes_len),
                    (READ_BENCH_JSON.to_owned(), case.json_bytes_len),
                ]),
            })
            .collect(),
    };
...
impl CaseSpec {
    pub(crate) fn prepare_read(&self) -> PreparedReadCase {
        ...
        let package_path = package_file.path().to_path_buf();
...
fn shared_corpus_root() -> PathBuf {
    std::env::var_os("CITYJSON_ARROW_SHARED_CORPUS_ROOT").map_or_else(
        || {
            panic!(
                "set CITYJSON_ARROW_SHARED_CORPUS_ROOT to your cityjson-corpus checkout, \
or set CITYJSON_ARROW_BENCHMARK_INDEX to the benchmark index path"
            )
        },
        PathBuf::from,
    )
