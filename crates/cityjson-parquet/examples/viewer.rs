...
struct SceneData {
    ...
    /// `cityobject_ix` → list of `geometry_id`s.
...
impl SceneData {
    ...
    fn triangulate_object(&self, cityobject_ix: u64) -> Vec<f64> {
        ...
        for &gid in geom_ids {
            ...
            for ring in surfaces {
                ...
                let v0 = self.vertex_swizzled(u64::from(ring[0]));
                for i in 1..ring.len() - 1 {
                    let v1 = self.vertex_swizzled(u64::from(ring[i]));
                    let v2 = self.vertex_swizzled(u64::from(ring[i + 1]));
    ...
    fn vertex_swizzled(&self, vid: u64) -> [f64; 3] {
        let v = self.vertices[usize::try_from(vid).expect("vertex index out of range")];
        [v[0], v[2], -v[1]]
    }
}
...
/// Decode surfaces from offset-based boundary encoding (v3alpha4).
///
...
fn decode_surfaces(
        ...
        && !ro.is_null(row)
    {
        ...
        for (si, &surf_start) in so_vals.iter().enumerate() {
            ...
            let surf_end = so_vals.get(si + 1).map_or(ro_vals.len(), |&v| v as usize);
        ...
        && !ro.is_null(row)
    {
        ...
        for (ri, &ring_start) in ro_vals.iter().enumerate() {
            ...
            let end = ro_vals
                .get(ri + 1)
                .map_or(vis.len(), |&v| (v as usize).min(vis.len()));
            surfaces.push(vis[start..end].to_vec());
        }
        return surfaces;
    }
