...
fn test_basic_attribute_operations() {
    ...
    attrs.insert("height".to_string(), AttributeValue::Float(42.5));
    attrs.insert("name".to_string(), AttributeValue::String("Tower".to_string()));
    attrs.insert("floors".to_string(), AttributeValue::Integer(10));
    attrs.insert("active".to_string(), AttributeValue::Bool(true));

    // Verify retrieval
    assert_eq!(attrs.len(), 4);

    if let Some(AttributeValue::Float(h)) = attrs.get("height") {
        assert_eq!(*h, 42.5);
    } else {
        panic!("Expected float value");
    }

    if let Some(AttributeValue::String(n)) = attrs.get("name") {
        assert_eq!(n, "Tower");
    } else {
...
fn test_cityobject_attributes() {
    ...
    if let Some(AttributeValue::Float(h)) = attrs.get("measuredHeight") {
        assert_eq!(*h, 25.5);
    }
}
...
fn test_semantic_attributes() {
    ...
    if let Some(AttributeValue::String(m)) = attrs.get("material") {
...
fn test_nested_map_attributes() {
    ...
    if let Some(AttributeValue::Map(addr)) = attrs.get("address") {
        assert!(addr.contains_key("street"));
        assert!(addr.contains_key("number"));

        if let Some(street_val) = addr.get("street") {
            if let AttributeValue::String(s) = street_val.as_ref() {
                assert_eq!(s, "Main St");
            }
        }
    }
...
fn test_nested_vector_attributes() {
    ...
    attrs.insert(
        "materials".to_string(),
        AttributeValue::Vec(materials)
    );

    // Verify vector access
    if let Some(AttributeValue::Vec(mats)) = attrs.get("materials") {
        assert_eq!(mats.len(), 3);

        if let AttributeValue::String(first) = mats[0].as_ref() {
...
fn test_attribute_modification() {
    ...
    attrs.insert("count".to_string(), AttributeValue::Integer(10));

    // Modify via get_mut
    if let Some(AttributeValue::Integer(c)) = attrs.get_mut("count") {
        *c += 5;
    }

    // Verify modification
    if let Some(AttributeValue::Integer(c)) = attrs.get("count") {
        assert_eq!(*c, 15);
    }
}
...
fn test_attribute_removal() {
    ...
    attrs.insert("temp".to_string(), AttributeValue::Bool(true));
...
fn test_cityobject_extra_attributes() {
    ...
    building.extra_mut().insert(
        ...
        AttributeValue::String("customValue".to_string())
    );
...
fn test_null_attribute_value() {
    ...
    // Test null values
    attrs.insert("nullable_field".to_string(), AttributeValue::Null);

    assert!(attrs.contains_key("nullable_field"));
    if let Some(AttributeValue::Null) = attrs.get("nullable_field") {
        // Expected behavior
    } else {
