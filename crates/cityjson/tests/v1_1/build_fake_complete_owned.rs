...
/// `tests/data/v1_1/cityjson_fake_complete.city.json`.
#[test]
fn build_fake_complete_owned() -> Result<()> {
    ...
    // This block scope is just for visual separation and code folding in the editor.
    {
        ...
        // Create address map with inline attributes
        ...
        let addresses_vec = vec![
            Box::new(AttributeValue::Map(address_map))
        ];
        co_1_extra.insert(
            "address".to_string(),
            AttributeValue::Vec(addresses_vec)
        );
        ...
        // a mutable borrow to the CityModel.
        {
            ...
            geometry_builder.map_vertex_to_uv(bv2, uv2);
            geometry_builder.map_vertex_to_uv(bv3, uv3);
            geometry_builder.set_texture_ring(None, texture_0, "theme-texture".to_string(), true)?;
    ...
    // Build CityObject "my-neighbourhood"
    {
        ...
        co_neigh_extra.insert(
            ...
            AttributeValue::Vec(children_roles)
        );
        {
            let mut geometry_builder =
