...
fn build_fake_complete_borrowed() -> Result<()> {
    ...
    *model.metadata_mut() = metadata;
    ...
    let mut co_tree = CityObject::new(
        CityObjectIdentifier::new(co_tree_id),
        CityObjectType::SolitaryVegetationObject,
    );
    let co_neighbourhood_id = "my-neighbourhood";
    let mut co_neighbourhood = CityObject::new(
        CityObjectIdentifier::new(co_neighbourhood_id),
        CityObjectType::CityObjectGroup,
    );

    // Create materials
    let mut material_irradiation = Material::new("irradiation");
    ...
    let material_red = Material::new("red");
    ...
    // This block scope is just for visual separation and code folding in the editor.
    {
        ...
        // Create address map attribute and vector for multiple addresses
        let addresses_vec = vec![Box::new(AttributeValue::Map(address_map))];
        co_1_extra.insert("address", AttributeValue::Vec(addresses_vec));
        ...
        // a mutable borrow to the CityModel.
        {
            ...
            let semantic_extension_type = "+PatioDoor";
    ...
    // Build CityObject "id-3".
    {
        ...
        co_3_attrs.insert("buildingLDenMin", AttributeValue::Float(1.0));
    }
    ...
    // Build CityObject "my-neighbourhood"
    {
        ...
        co_neigh_attrs.insert("location", AttributeValue::String("Magyarkanizsa"));
        ...
        co_neigh_extra.insert("children_roles", AttributeValue::Vec(roles_vec));
        {
            let mut geometry_builder =
...
fn build_metadata_with_reference(
    model: &mut CityModel<u32, BorrowedStringStorage<'static>>,
) -> Result<()> {
    let metadata_ref = model.metadata_mut();
    build_metadata(metadata_ref);
    Ok(())
...
fn build_metadata_with_return() -> Result<Metadata<BorrowedStringStorage<'static>>> {
...
fn build_metadata(metadata_ref: &mut Metadata<BorrowedStringStorage<'static>>) {
    metadata_ref
