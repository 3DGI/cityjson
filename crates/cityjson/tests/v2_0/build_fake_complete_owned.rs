...
struct Appearance {
    material_irradiation: OwnedMaterial,
    material_red: OwnedMaterial,
    texture_winter: OwnedTexture,
}

struct PendingCityObjects {
    building_part: OwnedCityObject,
    noise_building: OwnedCityObject,
    tree: OwnedCityObject,
    neighbourhood: OwnedCityObject,
}
...
fn assert_model_basics(model: &OwnedModel) {
...
fn assert_metadata_and_root(model: &OwnedModel) {
...
fn assert_model_assets(model: &OwnedModel, vertices: SharedVertices) {
...
fn assert_building_part_cityobject(model: &OwnedModel, refs: CityObjectRefs) {
    let co1 = model
        .cityobjects()
...
fn assert_building_part_address(co1: &OwnedCityObject) {
...
fn assert_building_part_geometry(model: &OwnedModel, co1: &OwnedCityObject) {
...
fn assert_noise_building_cityobject(model: &OwnedModel, refs: CityObjectRefs) {
    let co3 = model
        .cityobjects()
...
fn assert_tree_cityobject(model: &OwnedModel, v1: VertexIndex<u32>) {
    let co_tree = model
        .cityobjects()
        .iter()
...
fn assert_neighbourhood_cityobject(model: &OwnedModel, refs: CityObjectRefs) {
    let co_neigh = model
        .cityobjects()
...
fn build_metadata_patterns(model: &mut OwnedModel) {
...
fn build_root_components(model: &mut OwnedModel) {
    ...
    model
        .extra_mut()
        .insert("+census".to_string(), AttributeValue::Map(census_map));
...
fn build_shared_vertices(model: &mut OwnedModel) -> Result<SharedVertices> {
    Ok(SharedVertices {
...
fn build_cityobject_id_1(
    ...
    building_part: &mut OwnedCityObject,
...
fn set_cityobject_id_1_address(
    ...
    building_part
        .extra_mut()
        .insert("address".to_string(), AttributeValue::Vec(addresses_vec));
}

fn set_cityobject_id_1_attributes(building_part: &mut OwnedCityObject) {
...
fn add_cityobject_id_1_geometry(
    ...
    roof_semantic
        .attributes_mut()
        .insert("surfaceAttribute".to_string(), AttributeValue::Bool(true));
    ...
    let mut add_textured_surface = |semantic: Semantic<OwnedStringStorage>| -> Result<usize> {
    ...
    geometry_builder.set_semantic_surface(
        None,
        Semantic::new(SemanticType::Extension("+PatioDoor".to_string())),
        false,
    )?;
    geometry_builder.set_material_surface(
        None,
...
fn build_cityobject_id_3(noise_building: &mut OwnedCityObject) {
    noise_building
        .attributes_mut()
        .insert("buildingLDenMin".to_string(), AttributeValue::Float(1.0));
}

/// Build `CityObject` "a-tree" with template geometry and one geometry instance.
fn build_cityobject_tree(
    model: &mut OwnedModel,
    tree: &mut OwnedCityObject,
    vertices: SharedVertices,
) -> Result<()> {
...
fn build_cityobject_neighbourhood(
    model: &mut OwnedModel,
    neighbourhood: &mut OwnedCityObject,
    vertices: SharedVertices,
) -> Result<()> {
    neighbourhood.attributes_mut().insert(
        "location".to_string(),
    ...
    neighbourhood
        .extra_mut()
        .insert("children_roles".to_string(), AttributeValue::Vec(roles_vec));
...
fn link_semantics_for_schema_coverage(model: &mut OwnedModel) {
...
fn build_metadata_with_reference(model: &mut OwnedModel) {
...
fn build_metadata_with_return() -> OwnedMetadata {
...
fn build_metadata(metadata_ref: &mut OwnedMetadata) {
    ...
    address.insert(
        "city".to_string(),
        AttributeValue::String("Den Haag".to_string()),
    );
    address.insert(
        "country".to_string(),
        AttributeValue::String("The Netherlands".to_string()),
    );
