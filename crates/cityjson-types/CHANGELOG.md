...
### Changed
- **Breaking**: renamed the crate package from `cityjson` to `cityjson-types`.
  Rust imports now use the `cityjson_types` crate path, and the workspace crate
  now lives at `crates/cityjson-types`.
...
### Changed
- Replaced `cityjson_types::core` wildcard re-exports with explicit curated exports.
