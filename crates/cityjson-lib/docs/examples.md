...
The example programs mirror the intended public `cjlib` surface.
...
- `examples/explicit_json_module.rs`
  Use `cjlib::json` for probing, feature handling, and explicit boundary
  control.
...
- `examples/json_roundtrip.rs`
  Serialize documents and feature-sized models through `cjlib::json`.
