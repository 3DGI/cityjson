...
fn strongly_typed<P: AsRef<Path>>(
    path: P,
) -> serde_json::Result<Vec<nested::MultiSurfaceBoundary>> {
    let mut file = File::open(path.as_ref()).unwrap();
    let mut json_str = String::new();
    file.read_to_string(&mut json_str).unwrap();
    let ms: Vec<nested::MultiSurfaceBoundary> = serde_json::from_str(&json_str).unwrap();
    Ok(ms)
}
...
fn real_data(c: &mut Criterion) {
    ...
    let p_json = data_dir.join("boundaries.json");
    group.sampling_mode(sampling_mode);
    group.bench_function("strongly_typed", |b| {
        b.iter_with_large_drop(|| {
            let ms = strongly_typed(black_box(&p_json)).unwrap();
            black_box(ms);
        })
    });
    ...
    let mut group = c.benchmark_group("vertices");
    let p_json = data_dir.join("vertices.json");
    group.sampling_mode(sampling_mode);
    group.bench_function("strongly_typed", |b| {
        b.iter_with_large_drop(|| {
            ...
            let mut json_str = String::new();
            file.read_to_string(&mut json_str).unwrap();
            let ms: Vec<[i64; 3]> = serde_json::from_str(&json_str).unwrap();
            black_box(ms);
        })
    });
    group.bench_function("serde_json::Value", |b| {
    ...
    group.sampling_mode(sampling_mode);
    group.bench_function("strongly_typed:GeometryStruct", |b| {
        b.iter_with_large_drop(|| {
            let mut file = File::open(&p_json).unwrap();
            let mut json_str = String::new();
            file.read_to_string(&mut json_str).unwrap();
            let ms: nested::ModelGeomStruct = serde_json::from_str(&json_str).unwrap();
            black_box(ms);
        })
    });
    group.bench_function("strongly_typed:GeometryEnum", |b| {
        b.iter_with_large_drop(|| {
            let mut file = File::open(&p_json).unwrap();
            let mut json_str = String::new();
            file.read_to_string(&mut json_str).unwrap();
            let ms: nested::ModelGeomEnum = serde_json::from_str(&json_str).unwrap();
            black_box(ms);
        })
    });
    group.bench_function("serde_json::Value", |b| {
